...
//Проверить актуальность данных (все произвольно)ж
if( (summ = MainWindow->MoneyAshyper( FreeCheckForm->Summ->Text )) <= 0 )
  {
  summ = 0;
  if( MessageBox(NULL,"Сумма чека равна 0.00. Выбивать?","Печать свободного чека",MB_YESNO) == IDNO )return;
  }
if(summ > 2000000000 )
  {
  MessageBox(NULL,"Сумма чека не может быть больше 20 миллионов рублей, разделите ее на несколько чеков","Печать свободного чека",MB_OK);
  return;
  }

  //НДС с радиокнопок получим
...
//Пробить
    if(MainWindow->Star->Serial == "" || MainWindow->Star->SerialID == "") //Если серийный номер не определен, работать не можем!
      {
...
// Регистрируем в чеке строку продажи с указанными атрибутами
    MainWindow->Star->BillLine(FreeCheckForm->Text->Text.c_str(),FreeCheckForm->Summ->Text.c_str(),"1",1,"ШТ","",NDS);
